...
For more info about the package itself see `emishows/README.md`
...
(emishows) $ poetry install --extras dev
(emishows) $ emishows
```
...
To install the package, you need to `cd` into `emishows` directory and run:
...
Docs should be placed in `emishows/docs/docs`. They are pretty straightforward
to write.

To build the docs, `cd` into `emishows/docs` and run:
...
After that update the installation by running from `emishows` directory:
...
```sh
 docker build -t emishows .
```

To also run the container in one go, run:

```sh
docker build -t emishows . && docker run --rm -it emishows
```
